...
/*
void dump_memory(const char *name, unsigned int addr_lo, unsigned int addr_hi)
{
    unsigned int addr;
    
    putstr(debug_uart, name);    
    putstr(debug_uart, "\r\n");
        
    for (addr = addr_lo; addr < addr_hi; addr+=4)
    {
        unsigned int *ptr = (unsigned int *) addr;
        puthex32(debug_uart, addr);
        putstr(  debug_uart, " ");    
        puthex32(debug_uart, *ptr);
        putstr(  debug_uart, "\r\n");
    }
}
*/
...
static void ddr_timing_initialisation(const T_DDR_TIMING_PARAMETERS *ddr_timing_parameters)
{
    volatile UINT   reg_tmp = 0;    
...
#ifdef FPGA    
    reg_tmp = 7; // refresh holdoff = 7 clocks = 7*20 = 140ns
#else
...
static void ddr_normal_initialisation(const T_DDR_CONFIG_PARAMETERS *ddr_config_parameters, int mhz)
{
    int i;
    volatile UINT   tmp     = 0;
    volatile UINT   reg_tmp = 0;
    ...
    tmp = ddr_config_parameters->rams * ddr_config_parameters->asize * ddr_config_parameters->width;        
    //Deduce value to program into DDR_CFG register...
    switch ( tmp ) {
        case 16     : reg_tmp = 0x00020000 * 1; break;
        case 32     : reg_tmp = 0x00020000 * 2; break;
        case 64     : reg_tmp = 0x00020000 * 3; break;
        case 128    : reg_tmp = 0x00020000 * 4; break;
        case 256    : reg_tmp = 0x00020000 * 5; break;
        case 512    : reg_tmp = 0x00020000 * 6; break;
        case 1024   : reg_tmp = 0x00020000 * 7; break;
        case 2048   : reg_tmp = 0x00020000 * 8; break;
        default     : reg_tmp = 0; //forces sims not to work if badly configured
    }

    //Memory width
    tmp = ddr_config_parameters->rams * ddr_config_parameters->width;        
    switch ( tmp ) {
        case 8      : reg_tmp = reg_tmp + 0x00400000; break;
        case 16     : reg_tmp = reg_tmp + 0x00200000; break;
        case 32     : reg_tmp = reg_tmp + 0x00000000; break; 
        default     : reg_tmp = 0; //forces sims not to work if badly configured       
    }

    //Setup DDR Mode
    switch ( ddr_config_parameters->ddr_mode ) {
        case 0      : reg_tmp = reg_tmp + 0x00000000; break;   //SDR
        case 1      : reg_tmp = reg_tmp + 0x40000000; break;   //DDR
        case 2      : reg_tmp = reg_tmp + 0x80000000; break;   //DDR2
        default     : reg_tmp = 0; //forces sims not to work if badly configured       
    }        
    ...
    * (volatile UINT *) C_DDR_CFG_REG = reg_tmp;    
    ...
    switch ( ddr_config_parameters->ddr_mode ) {
        case 0      : reg_tmp = 2 + (0 << 4); break;   //SDR
        case 1      : reg_tmp = 2 + (4 << 4); break;   //DDR
        case 2      : reg_tmp = 0 + (4 << 4); break;   //DDR2
        default     : reg_tmp = 0;
    }     
...
/*        case 0      :   //SDR
                        ...
                        //Run dll_clk with DLL turned off and with a small delay (was zero, now adjusted so that netlist simulation works)...
                        reg_tmp = 0x80000000 + 1;
                        * (volatile UINT *) C_DDR_DLL_REG = reg_tmp;    

                        ddr_poll_until_not_busy ();
                        //Startup SDRAM...
                        //!!! Software: CK should be running for 100us before wake-up
        ...
        case 1      :   //DDR
                        ...
                        * (volatile UINT *) C_DDR_REG_PHY2 = reg_tmp;    
                        ...
                        ddr_issue_command ( C_CMD_WAKE_UP );
                        ddr_issue_command ( C_CMD_PRECHARGE_ALL );
                        ddr_issue_command ( C_CMD_DDR2_EMR1 + C_CMD_ENABLE_DLL );
                        //Sets CL=2.5; BL=8 but also reset DLL to trigger a DLL initialisation...
                        ddr_issue_command ( ddr_config_parameters->cmd_mode_wr_cl_bl + C_CMD_RESET_DLL );
        ...
        case 2      :   //DDR2
                        ...
                        * (volatile UINT *) C_DDR_REG_PHY2 = reg_tmp;    
                        ...
                        #ifdef FPGA
                        emr_cmd = C_CMD_DDR2_EMR1 + C_CMD_ODT_DISABLED + C_CMD_REDUCED_DRIVE + C_CMD_DISABLE_DLL;
                        #else
                        emr_cmd = C_CMD_DDR2_EMR1 + C_CMD_ODT_75 + C_CMD_REDUCED_DRIVE + C_CMD_ENABLE_DLL;
                        #endif
...
{
    static const T_DDR_TIMING_PARAMETERS *ddr_timing_parameters = &C_TP_DDR2_25_CL6_1GB;
    static const T_DDR_CONFIG_PARAMETERS *ddr_config_parameters = &C_CP_DDR2_25_CL6;
    ...
    * (volatile UINT *) C_DDR_BLKEN_REG = C_BLKEN_DDR_ON + 0x0000FFFF;
    ...
    //dump_memory("DDR Core Registers", DDR_BASE,         DDR_BASE+0x70);
    //dump_memory("DDR PHY Registers",  DDR_BASE+0x80000, DDR_BASE+0x800CC);    
}
...
void set_ddr_timing(unsigned int w, unsigned int i) {
        ...
        while (wnow != w) {
            ...
            writel(DDR_PHY_TIMING_CK | DDR_PHY_TIMING_W_CE | reg, DDR_PHY_TIMING);
            writel(DDR_PHY_TIMING_W_CE | reg, DDR_PHY_TIMING);
        }
        ...
        while (inow != i) {
            ...
            writel(DDR_PHY_TIMING_CK | DDR_PHY_TIMING_I_CE | reg, DDR_PHY_TIMING);
            writel(DDR_PHY_TIMING_I_CE | reg, DDR_PHY_TIMING);
        }
}
