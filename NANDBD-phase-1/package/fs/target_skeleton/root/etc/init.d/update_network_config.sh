...
INTERFACES=$( ip address show | awk -v ORS=" " '$1 == "inet" {print $2}' )
sed -i -e "s:interfaces[ ]*=.*\$:interfaces=$INTERFACES:" $SMBconf
sed -i -e "s:workgroup[ ]*=.*\$:workgroup=$workgroup:" $SMBconf
sed -i -e "s:server string[ ]*=.*\$:server string=$current_hostname:" $SMBconf
