...
sanityCheck()
{
    ...
    [ -z $1 ] || [ -z $2 ] && quit "usage: $0 disk version-string"
    ...
    [ -r "$rootfs" ] || [ -r "$customerfsbzip" ] || quit "root filesystem image isn't readable"
...
create_partitions()
{
    ...
    parted $disk mkpart primary 9502720s 345047039s
    parted $disk mkpart primary 345047040s 100%
    parted $disk set 5 raid on
    parted $disk print
...
start_usr_raid()
{
    ...
    mdadm --create $MD4 --auto=md --raid-devices=2 --level=raid1 --run --metadata=0.90 "${disk}5" missing
}
...
format_usr_partitions()
{
    ...
    mkfs.xfs -f -l lazy-count=1 "${disk}4"
...
start_sys_raid $1
format_sys_partitions
copyRootFilesystem "$2" "1nc"
stop_raid
start_usr_raid $1
format_usr_partitions $1
sleep 2
stop_raid
